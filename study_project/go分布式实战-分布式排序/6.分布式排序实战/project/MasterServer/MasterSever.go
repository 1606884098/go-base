...
func main() {
	...
	//切割
	N := len(iplist)
	savelist := SplitFile(path2, pathresult, N)
	fmt.Println("切割完成", savelist) //切割有效的
	//建立链接池
	connlist := []net.Conn{} //连接池
	...
	for i := 0; i < len(connlist); i++ {
		go func(i int) {
			SendFile(savelist[i], connlist[i], 4, true, false)
			fmt.Println("发送完成", connlist[i])
		}(i)
	}
	...
	for i := 0; i < len(connlist); i++ {
		...
		lastsavelist = append(lastsavelist, savepath)
		go func(i int) {
			ReceFile(savepath, connlist[i], 4)
			fmt.Println("发送完成", connlist[i])
			wg.Done()
		}(i)
	}
	...
	fmt.Println(Merge(lastsavelist, pathresult, 4, true, "csdnPasswdRmDuplicatemidd"))
	fmt.Println("归并完成")
	// waitGroup   归并
